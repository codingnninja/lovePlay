...
const getSong = async (index) => {
  ...
  if (appState.shuffle && appState.selected === false) {
    ...
    if(completed) $render(Play, { song, playing, trigger });
    return true;
...
const Volume = ({ song }) => {
  ...
  const volume = appState.volume ? appState.volume : 1;
  return `
...
const Play = ({ song, playing, trigger }) => {
  ...
  function pauseOrPlay (playing) {
    ...
    if (playing) {
      audio.volume = appState.volume ? appState.volume : 1;
      audio.play();
...
const Shuffle = () => {
  const shuffle = () => {
    appState.repeat = appState.repeat && false;
    $render(Repeat);
...
const Repeat = () => {
  const repeat = () => {
    appState.shuffle = appState.shuffle && false; 
    $render(Shuffle);
...
const appRendered = await $render(App, { songs, toggle });
...
const appView = await $render(App, {songs, toggle});
  // Swiping implementation
  let touchArea = $select("#root");
